...
public class InternalEmployeeControllerTests
{
    ...
    [Fact]
    public async Task GetInternalEmployees_ReturnOkObjectResultWithCorrectAmountOfInternalEmployees()
    {
        var result = await _internalEmployeesController.GetInternalEmployees();

        var actionResult = Assert.IsType<ActionResult<IEnumerable<Models.InternalEmployeeDto>>>(result);

        var okObjectResutl = Assert.IsType<OkObjectResult>(actionResult.Result);
        var dtos = Assert.IsAssignableFrom<IEnumerable<Models.InternalEmployeeDto>>(okObjectResutl.Value);
